...
export default function EditSimulationModal({
  ...
  useEffect(() => {
    if (simulation) {
      ...
      setSteps(
        simulation.steps?.length ? simulation.steps : [{ description: "" }],
      );
      setInitialFiles(
      ...
      const solutionEntries = simulation.solution
        ? Object.entries(simulation.solution).map(([fileName, code]) => ({
            fileName,
            code,
          }))
        : [{ fileName: "", code: "" }];
      setSolutionFiles(
        solutionEntries.length ? solutionEntries : [{ fileName: "", code: "" }],
      );
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full my-8">
        ...
        >
          ...
          {/* Title */}
          <div className="mb-6">
            <label className="block font-mono text-sm text-gray-700 mb-2 tracking-wide">
              SIMULATION TITLE *
            </label>
            <input
              type="text"
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                    ...
                    <select
                      value={file.language}
                      onChange={(e) =>
                        updateFile(i, "language", e.target.value)
                      }
                      className="px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-black"
                    >
