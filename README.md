...
| ------------------------ | -------------------------------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------- |
| **Home**                 | `page.tsx`                             | Landing page with marketing content | `HeroSection`, `HowItWorksSection`, `LatestIncidentsSection`, `BenefitsSection`, `WhyEnumSection` |
...
#### **Simulations** (`simulation.route.js`)
...
- `auth.middleware.js` — `verifyJWT` (required auth), `optionalAuth` (optional
  JWT parsing)
...
- `certs` (UserCert[]) — Certifications (name, issuer, date, done)
- **XP System**:
  - `xp` (Int) — Total experience points
  - `browserXpClaims` (Int) — XP claimed from browser activity
  - `currentStreak` (Int) — Consecutive days active
  - `lastActivityDate` (DateTime) — Last activity timestamp
- **Relations**: Solutions[], Submissions[], SimulationProgress[],
...
- `constraints` (String)
- `topic` (String) — DSA topic (arrays, linked lists, etc.)
...
- `code` (String)
- `language` (String)
- `verdict` (String) — "accepted", "wrong_answer", "runtime_error",
  "compile_error", "error", "partial"
...
- `timeComplexity`, `spaceComplexity` (String) — e.g., "O(n)", "O(1)"
- `createdAt`, `updatedAt` (DateTime)
...
- `initialFiles` (SimulationFile[]) — File structure with content, language,
  Cloudinary URLs
...
- **Docker Execution**: Sends code to external compiler service
  (`enumcompiler.duckdns.org`)
...
- **Rule-based Scoring**: Each rule has points (e.g., "include load balancer" =
  2 pts)
...
- **Configuration**: `utils/cloudinary.js` initializes with
...
- **In-Memory Rooms Map**:
  `Map<roomId, Map<socketId, {userId, username, color, tabId}>>`
...
1. **XP System**:
   - Awarded on problem solving, simulations, system design
   - Scales by difficulty
   - Browser activity XP claims
...
│  ├─ Simulations (File Explorer)      │
│  ├─ System Design (React Flow)       │
│  ├─ Collab Editor (Socket.IO)        │
│  └─ Dashboard (User Stats)           │
└────────────────────────────────────┬─┘
         ↓          ↑
      HTTP API    Socket Events
...
└────────────────────────────────────┬─┘
         ↓          ↓          ↓
    MongoDB   Cloudinary   Docker Service
    (Data)    (Files)      (Execution)
         ↑
      Redis (Cache, Queue)
```
