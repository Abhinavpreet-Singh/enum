    ...
    examples: [
      {
        ...
        output: "/app",
        explanation: "The sandbox starts in /app, so pwd should print that path.",
      },
    ],
    starterCode: "#!/usr/bin/env bash\n# Print the current directory\n",
    expectedOutput: "/app",
    constraints: ["Use a single Bash command.", "Do not hardcode the answer as plain text."],
    ...
    examples: [
      {
        ...
        output: ".arena-secret\ncode.sh",
        explanation: "The sandbox includes a hidden marker file and the script itself.",
      },
    ],
...
async function seed() {
  try {
    ...
    let created = 0;
    for (const question of linuxQuestions) {
      const existing = await prisma.linuxQuestion.findUnique({
        where: { slug: question.slug },
      });

      if (existing) {
        console.log(`Skipping ${question.slug} (already exists)`);
        continue;
      }

      await prisma.linuxQuestion.create({ data: question });
      console.log(`Created ${question.slug}`);
      created += 1;
    }

    console.log(`Seeded ${created} Linux questions.`);
  } catch (error) {
